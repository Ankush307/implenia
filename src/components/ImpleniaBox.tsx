                ...
                <div className="flex items-center justify-center gap-x-[51px] flex-wrap ">
                    {KICKBOX_PROCESS_LIST.map((item, i) => (
                        <div key={i} className="flex flex-col items-center justify-center">
                            <Image className='lg:pt-[75px] pt-10 lg:pb-[39px] pb-6 ' src={item.image} alt={item.imageAlt} width={227} height={92}></Image>
                            <div className="border border-black p-[28px_20px_19px_26px] rounded-[28px]">
                                <p className={`text-[21px] leading-[21px] pb-1 font-semibold ${i === 0 ? "text-light-red " : i === 1 ? "text-light-black" : "text-yellow"}`}>{item.subTitle} </p>
                                <p className='text-[35px] font-semibold leading-[44px] '>{item.title}</p>
                                <p className='font-light text-light-black text-[21px] leading-[27px] tracking-[-0.28px] max-w-[277px] pt-3'>{item.description}</p>
                                <p className='font-light text-light-black text-[21px] leading-[27px] tracking-[-0.28px] max-w-[277px] pt-[30px]'>{item.boxDuration}</p>
                                <p className='font-light text-light-black text-[21px] leading-[27px] tracking-[-0.28px] max-w-[257px]'>{item.budget}</p>
                            </div>
                        </div>
                    ))}
                </div>
                <div className="w-full max-w-[268px] max-xl:mx-auto">
                    <CustomButton myClass="text-light-red border-light-red py-4 px-8 min-w-[268px] font-bold font-sans-bold hover:bg-light-red hover:text-white mt-10 ml-10 max-lg:text-center max-lg:ml-0 max-lg:mt-6" text="Start your RedBox!" />
                </div>
            </div>
        </div>
    )
